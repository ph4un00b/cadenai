...
function Arrow(props: {
	...
	let length = Math.sqrt(
		(props.target.x - props.origin.x) ** 2 +
			(props.target.y - props.origin.y) ** 2,
	);
	...
	let arrowPath = `M ${points[0]![0]} ${points[0]![1]} L ${points[1]![0]} ${
		points[1]![1]
...
function Tree(props: { root: YTreeNode }) {
	...
	return (
		...
		>
			...
			{tentativeEdgePath && (
				<Arrow
					origin={tentativeEdgePath[0]}
					target={tentativeEdgePath[1]}
					animate={false}
				/>
			)}
			...
			{nodes.map((node) => (
				...
				>
					<circle
						...
						stroke={
							node === dragOrigin || node === dragTarget ? "white" : "black"
						}
						strokeWidth={2}
					/>
					<text
	...
	return (
		<div className="space-y-3 p-4 sm:p-8">
			<Paragraph>
				...
				on top of Yjs. Double-click a node to add a child; drag a node to
